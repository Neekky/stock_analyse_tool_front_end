...
import { limitupApi, allInfoApi } from "@/apis";
  ...
  const columns = useMemo<ColumnsType<any>>(() => {
    return [
      ...
      },
      {
        ...
        key: "集合竞价评级",
        render: (text) => (
          <div style={{ color: text === "看多" ? "red" : "green" }}>{text}</div>
        ),
      },
      {
        title: "当日收盘涨幅",
...

      {
        ...
        render: (_, record: { key: React.Key }) => (
          <Button
            ...
            onClick={() => wrapHandleViewDetail(record.key)}
          >
            <a>详情</a>
          </Button>
        ),
  ...
  const pageGetEarlyLimit = async (queryDate) => {
    ...
    try {
      ...
      if (res.code !== 200) {
        ...
        const firstDealData = res.data?.map((ele) => ({
          ...ele,
          key: ele["股票简称"],
        }));
